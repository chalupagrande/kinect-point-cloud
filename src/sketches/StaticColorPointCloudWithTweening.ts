...
export default function PointCloud(canvas: HTMLCanvasElement) {
    ...
    setup(canvas)
    let previousPos: THREE.Vector3 | undefined;
    window.addEventListener('resize', onWindowResize, false)
    window.addEventListener("keydown", (e) => {
        console.log(previousPos, camera.position)
        if (e.key === "ArrowLeft") {
            if (!previousPos) {
                console.log("SETTING PREVIOUS POS")
                previousPos = camera.position.clone()
            }
            else if (!tween) {
                console.log("SETTING TWEEN")
                tween = new TWEEN.Tween(previousPos).to(camera.position.clone(), 1000)
                previousPos = camera.position.clone()
            } else {
                console.log("ADDING TWEEN")
                tweens.push(new TWEEN.Tween(previousPos).to(camera.position.clone(), 1000))
            }
            previousPos = camera.position.clone()
            ...
            for (let i = 0; i < tweens.length; i++) {
                curTween.chain(tweens[i])
                curTween = tweens[i]
                curTween.onUpdate((obj: TWEEN.Tween) => {
